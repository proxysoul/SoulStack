...
- Ecosystems and lockfiles: `site/package.json` + `site/bun.lock` (the website only; the setup scripts have no dependencies).
- Wired into `immune:smoke`: secret patterns over tracked files (or `gitleaks dir` when installed), https-only downloads in both setup scripts, `bun audit` for the site.
- Installers: `scripts/setup.sh` and `setup.ps1` clone over https and never run downloaded code except the repo itself; presets read tokens from the environment, never from files.
- Where secrets could appear: `plugins/presets/*.json`, screenshots under `assets/`, the site's docs.
